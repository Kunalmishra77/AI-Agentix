...
router.post('/', validate, async (req, res, next) => {
  ...
  try {
    ...
    const submission = await Contact.create({
      name, email, company, phone, service, message, budget, source,
      ip: req.ip,
      ...
      html: `
        <div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto">
          ...
          <div style="padding:32px;background:#f7f7f5;border:1px solid #e5e5e5">
            ...
            <p style="color:#999;font-size:12px;margin-top:20px">Submission ID: ${submission._id}</p>
          </div>
        </div>
      `,
    };
...
router.put('/:id/status', protect, async (req, res, next) => {
  try {
    const sub = await Contact.findByIdAndUpdate(
